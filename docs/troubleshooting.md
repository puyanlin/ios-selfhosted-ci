...
| `Simulator did not finish booting within 5 minutes`, jobs then hang until cancelled | Machine overloaded (several simulators booting at once, long-running simulators left open, VMs) | Simulator tests are serialized per machine with a lock (`~/actions-runners/_locks`); cleanup is time-boxed. Check `uptime` load and shut down simulators you left booted |
