...
在有憑證的那台 Mac：鑰匙圈存取 › **登入** › **我的憑證** › 在「Apple Distribution: <公司> (<TEAMID>)」按右鍵 ›
**輸出** › 格式選「個人資訊交換 (.p12)」，設一組密碼。把 `.p12` 和 `.mobileprovision` 用 AirDrop 傳到 runner，
然後在 runner 的 **Terminal.app** 執行（要輸入密碼的步驟不要透過 AI agent）：

```bash
scripts/ci-signing-setup.sh keychain                    # 每台 Mac 一次（做過就不用）
scripts/ci-signing-setup.sh p12 ~/Desktop/company.p12   # 會問 .p12 的密碼
scripts/ci-signing-setup.sh profile ~/Desktop/*.mobileprovision
rm ~/Desktop/company.p12                                # 裡面有私鑰，用完刪掉
```
...
或在 repo 的 `.github/workflows/testflight.yml` 加上 `signing: manual`。TestFlight job 會：
